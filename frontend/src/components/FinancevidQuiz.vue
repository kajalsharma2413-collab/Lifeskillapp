    ...
    <section class="max-w-4xl mx-auto p-6 text-white">
        <div ref="card" :style="cardStyle" class="glass-box animate-fade-in">
            <div v-if="questions.length && !showScore">
                ...
                <!-- Question -->
                <h2 class="text-xl font-extrabold text-green-300 mb-4 text-center">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <button
                        class="px-6 py-2 rounded-full bg-green-400 text-black font-bold hover:bg-green-300 transition"
                        @click="nextQuestion">
                        Next ➡
                    </button>
                </div>
            ...
            <div v-else-if="!showScore && !questions.length" class="text-center py-12">
            ...
            <div v-if="showScore" class="mt-10 text-center space-y-4">
                <h3 class="text-2xl font-bold text-green-300">💡 Your Score</h3>
...
onMounted(async () => {
    try {
        ...
        quizId.value = data.data.metadata.quiz_id
        questions.value = data.data.questions
    } catch (err) {
        console.warn('⚠️ Could not load quiz:', err.message)
    }
})

const nextQuestion = () => {
    if (!selectedAnswer.value) return alert("Please select an answer")
...
const finishQuiz = async () => {
    const token = localStorage.getItem("firebase_token");
    try {
        const res = await fetch(`/api/v1/quiz/${quizId.value}/submit`, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                time_taken_seconds: 0, // optional, can measure with timer
            }),
        })
