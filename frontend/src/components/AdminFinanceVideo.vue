<template>
    <div class="flex-1 p-10">
        <!-- Finance Videos Section -->
        <div>
            <!-- Video List -->
            <div class="glass-box mb-6">
                <h3 class="text-xl font-bold mb-4">🎥 Existing Finance Videos</h3>
                <div v-for="video in videoList" :key="video.id"
                    ...
                    <!-- Video -->
                    <video v-if="video.video_url" controls class="w-full mt-2 rounded-lg"
            ...
            <!-- Add Video Form -->
            <div class="glass-box mb-6">
                <h3 class="text-2xl font-bold mb-4">➕ Add Finance Video</h3>
            ...
            <div v-if="selectedVideo" class="glass-box mb-6">
                ...
                <form @submit.prevent="addVideoQuestion" class="space-y-4">
                    <input v-model="videoQuestion.question_text" placeholder="Enter question" class="form-input"
                        required />
                ...
                <div v-if="videoQuiz.questions?.length" class="mt-4 space-y-4">
                    <div v-for="(question, qIdx) in videoQuiz.questions" :key="question.id" class="glass-box">
                        ...
                        <div v-if="currentQuestionId === question.id" class="mt-3">
                            <input v-model="videoOption.option_text" placeholder="Option Text"
...
async function assignBadgeToQuiz(quizId, badgeId) {
    console.log("🚀 Assigning badge to quiz:", { quizId, badgeId });  // Debug log
    if (!quizId || !badgeId) {
        console.warn("❌ Missing quizId or badgeId", { quizId, badgeId });
        return;
    }
    try {
        ...
        console.log("🏅 Badge assigned:", data);
        return data.success;
    } catch (err) {
        console.error("❌ Error assigning badge to quiz", err);
        return false;
    }
}
...
async function addVideo() {
    try {
        const res = await fetch("/api/v1/admin/finance/video", {
            method: "POST",
...
async function deleteVideo(videoId) {
    ...
    try {
        const res = await fetch(`/api/v1/admin/finance/video/${videoId}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
...
async function openQuiz(video) {
    ...
    if (video.quiz_id) {
        ...
        // 🔥 Automatically assign badge before loading questions
        if (video.badge_id) {
            await assignBadgeToQuiz(video.quiz_id, video.badge_id);
        }
...
async function addVideoQuestion() {
    ...
    try {
        const res = await fetch(`/api/v1/admin/finance/video/${selectedVideo.value.id}/question`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
...
async function addVideoOption(questionId) {
    try {
        ...
        const res = await fetch(`/api/v1/admin/finance/video/question/${questionId}/option`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
